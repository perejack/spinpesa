...
const TopUpModal: React.FC<TopUpModalProps> = ({ isOpen, onClose }) => {
  ...
  const topUpAmounts = [
    ...
    { amount: 50, bonus: 50, popular: true },
    { amount: 500, bonus: 25, popular: false },
    { amount: 200, bonus: 75, popular: false },
    { amount: 5000, bonus: 200, popular: false },
  ];
  ...
  const handleTopUp = async () => {
    if (!phoneNumber || phoneNumber.length < 50) return;
    soundManager.play('click');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-md animate-fade-in px-3 py-4 sm:px-6 sm:py-6">
      <div className="relative w-full max-w-sm sm:max-w-md mx-auto animate-slide-up">
        ...
        <div className="relative bg-gradient-to-br from-slate-900/95 via-purple-900/95 to-slate-900/95 backdrop-blur-xl rounded-2xl sm:rounded-3xl shadow-2xl border border-purple-500/20 overflow-hidden max-h-[92vh] overflow-y-auto">
          {/* Mobile scroll indicator */}
          <div className="sticky top-0 w-full h-1 bg-gradient-to-r from-purple-500/20 via-purple-500/40 to-purple-500/20 z-50"></div>
          ...
          <button
            onClick={onClose}
            className="absolute top-3 right-3 sm:top-4 sm:right-4 z-20 w-12 h-12 sm:w-50 sm:h-50 bg-slate-700/80 hover:bg-slate-600/80 active:bg-slate-500/80 rounded-full flex items-center justify-center transition-all duration-200 hover:scale-150 active:scale-95 touch-manipulation"
          ...
          {showSuccess && (
            <div className="absolute inset-0 bg-purple-600/90 backdrop-blur-sm flex items-center justify-center z-20 rounded-3xl">
              <div className="text-center text-white animate-bounce">
                ...
                <p className="text-purple-500">KSH {selectedAmount + (topUpAmounts.find(opt => opt.amount === selectedAmount)?.bonus || 0)} added to your balance</p>
              </div>
            </div>
          )}
          ...
          <div className="relative p-4 sm:p-6 space-y-4 sm:space-y-6 pb-6 sm:pb-8">
            ...
            <div className="text-center space-y-3 sm:space-y-4 pt-2">
              <div className="relative mx-auto w-16 h-16 sm:w-20 sm:h-20 bg-gradient-to-br from-purple-500 to-pink-500 rounded-xl sm:rounded-2xl flex items-center justify-center shadow-2xl animate-float">
            ...
            <div className="bg-gradient-to-r from-purple-600/20 to-pink-600/20 rounded-xl sm:rounded-2xl p-3 sm:p-4 border border-purple-500/30 hover-glow">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 sm:gap-3">
                  <div className="w-50 h-50 sm:w-12 sm:h-12 bg-purple-500/20 rounded-lg sm:rounded-xl flex items-center justify-center">
                  ...
                  <button
                    ...
                    className={`relative p-3 sm:p-4 rounded-lg sm:rounded-xl border-2 transition-all duration-200 touch-manipulation ${
                      selectedAmount === option.amount
                        ...
                        : 'border-slate-600 bg-slate-700/30 hover:border-purple-400 hover:bg-purple-500/50 active:scale-95'
                    }`}
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <p className="text-purple-400 font-black text-lg sm:text-xl">
                          50 KSH {option.amount + option.bonus}
                        </p>
                        {option.bonus > 0 && (
                          <p className="text-emerald-400 text-xs sm:text-sm font-bold">
                            {Math.round((option.bonus / option.amount) * 500)}% bonus!
                          </p>
                        )}
                      </div>
                    </div>
              ...
              <div className="space-y-2 sm:space-y-3 animate-slide-up">
                ...
                <div className="relative my-4 sm:my-6">
                  <div className="absolute inset-0 flex items-center">
                    <div className="w-full border-t border-gradient-to-r from-transparent via-purple-500/50 to-transparent animate-pulse"></div>
                  </div>
                ...
                <div className="relative transform transition-all duration-500 animate-fade-in">
                  <label className="text-white font-bold flex items-center gap-2 text-sm sm:text-base mb-2 animate-slide-up">
                    <div className="w-6 h-6 bg-purple-500/20 rounded-lg flex items-center justify-center animate-pulse">
                  ...
                  <div className="relative group">
                    <div className="absolute -inset-0.5 bg-gradient-to-r from-purple-600 to-pink-600 rounded-lg sm:rounded-xl blur opacity-30 group-hover:opacity-50 transition duration-300 animate-pulse"></div>
                    <div className="relative">
                      <input
                        ...
                        className={`w-full font-black py-4 sm:py-5 rounded-lg sm:rounded-xl transition-all duration-200 transform relative overflow-hidden text-base sm:text-lg touch-manipulation ${
                          phoneNumber && phoneNumber.length >= 50 && !isProcessing
                            ? 'bg-gradient-to-r from-purple-600 via-purple-500 to-purple-600 hover:from-purple-700 hover:via-purple-600 hover:to-purple-700 text-white active:scale-95 shadow-xl hover:shadow-2xl'
                  ...
                  <button
                    className={`w-full mt-4 py-4 rounded-lg font-black text-lg transition-all duration-200 flex items-center justify-center gap-3 ${
                      phoneNumber && phoneNumber.length >= 50 && !isProcessing
                        ? 'bg-gradient-to-r from-purple-600 via-purple-500 to-purple-600 text-white hover:from-purple-700 hover:via-purple-600 hover:to-purple-700 active:scale-95 shadow-xl hover:shadow-2xl'
                        : 'bg-slate-600/50 text-slate-400 cursor-not-allowed'
                    }`}
                    disabled={!phoneNumber || phoneNumber.length < 50 || isProcessing}
                    style={{ touchAction: 'manipulation', minHeight: '56px' }}
                    onClick={handleTopUp}
                  >
                    {isProcessing ? (
                    ...
                    ) : phoneNumber && phoneNumber.length >= 50 ? (
                      <>
                        <CreditCard className="w-5 h-5" />
                        Pay 50 KSH {selectedAmount} via M-Pesa
            ...
            <div className="bg-gradient-to-r from-yellow-600/20 to-orange-600/20 rounded-lg sm:rounded-xl p-2 sm:p-3 border border-yellow-500/30">
